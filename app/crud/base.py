...
class CRUDBase[ModelType: BaseModel, CreateSchemaType: SQLModel, UpdateSchemaType: BaseModel]:
    ...
    def safe_get(self, db: Session, obj_id: UUID, *args, **kwargs) -> ModelType:
        data = self.get(db, obj_id)
        if data:
            logger.info(f"Retreived {self.model.__name__}: {obj_id}")
            return data
        raise NotFoundException

    def get(self, db: Session, obj_id: UUID, *args, **kwargs) -> ModelType | None:
        statement = select(self.model).where(self.model.id == obj_id, self.model.enabled == True)
    ...
    def get_all(
        ...
        *,
        ...
        statement = select(self.model).where(self.model.enabled == True).limit(limit)
        obj = list(db.exec(statement).all())
        logger.info(f"Retreived {len(obj)} {self.model.__name__}s")

        return obj

    def paginated_get_all(self, db: Session) -> Page[ModelType]:
        statement = select(self.model).where(self.model.enabled == True).order_by(self.model.created_at)
    ...
    def remove_many(
        ...
        *,
        ...
        logger.info(f"{'Hard deleted' if hard else 'Removed'} {len(db_objs)} {self.model.__name__} objects")
    ...
    def remove(
        ...
        *,
        ...
        logger.info(f"{'Hard deleted' if hard else 'Removed'} {self.model.__name__}: {db_obj.id}")
